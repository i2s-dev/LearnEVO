...
Status: **partial** — Pass 368-374/565: library-expansion zone, OP_C0=BLOCK_OPEN/OP_C1=BLOCK_CLOSE confirmed; OP_44=ENTER_EXEC_BEGIN 75%; OP_56=DATA_CONTINUATION confirmed; OP_19=BROWSE_COLUMN_DESCRIPTOR 70%; OP_1B=LARGE_DESCRIPTOR_BODY 50%; OP_5D=INV_MENU_DESCRIPTOR 55% (Pattern B confirmed inv_menu source; Pattern A origin unconfirmed)
...
**Pass 565 update (2026-07-03): OP_5D source keyword narrowed to `inv_menu` (MAGLIB):**
- Cross-referenced `inv_menu` call sites with OP_5D counts across all 3 Rosetta Stone files:
  - BKLME: 2 `inv_menu` calls (lines 772, 779), 0 args → 1 library-zone OP_5D pair (I#1293-1294)
  - BKROA: 1 `inv_menu` call (line 228, args: `' ',icol[which],irow[which]+1`) + 1 standalone definition → 1 library-zone OP_5D pair (I#2001-2002) + 1 startup OP_5D (Pattern A)
  - BKDCA: 0 `inv_menu` calls → 0 OP_5D
- `inv_menu` is the MAGLIB multi-column scrollable browser. All three files include `#LIB LOOKUPS`, but only BKROA includes `#LIB MAGLIB`. BKLME does NOT include `#LIB MAGLIB` yet still has `inv_menu` — `inv_menu` must be available from `#LIB DBA` (common to BKROA, BKLME, BKDCA), not exclusively from MAGLIB.
- **Pattern B revised interpretation:** The [OP_16(4)][OP_5D(20)][OP_5D(20)] library stub = the INV_MENU browse configuration descriptor (64 bytes total). BKROA's address-table content (sub-records `[0A][addr][01 00 0F 00]`) points to individual OP_19 browse column records; BKLME's field-offset content (0x0F60=3936, 0x0B10=2832 with type-tags) describes the record fields to browse. These are the same structure in two different display configurations.
- **Pattern A revised:** Still BKROA-only at I#29 startup. BKROA is the only file with `#TDATA 256000`; BKLME has 0 `#TDATA`. Pattern A may be the TDATA buffer initialization descriptor (20-byte zero-filled startup block). Unconfirmed — 35% confidence for this specific attribution.
- **Upgraded confidence: Pattern B = 65%, Pattern A = 35%, combined = 55%.**

**Remaining unknowns (Pass 374/565):** OP_5D Pattern B source keyword now identified as `inv_menu`; Pattern A attribution to `#TDATA` unconfirmed. OP_19/OP_1B at 70%/50% still have gaps. No remaining fully-opaque opcodes after these.
