...
- [x] 🔄 **RT** — Report Template Validator (T7RTMVALID: 20 procs confirmed Pass 116, 68,677 bytes, src=NZLICE.LIB); opens BKSYHELP+DBAHLPID+ISIS+MKAHIST as its own 4 DB files; ISTS.PATH (network share path), ISTS.EDATE (effective date); ISIS = import/export index (scans RTM files); Pass 160: **ISTXG.HNDL/ISTXF.HNDL/ISTAX.HNDL/ISHTX.HNDL/HTAX.HNDL/ISMCF.HNDL/ISMCR.HNDL/ISDUTY.HNDL** = 8 globally-opened tax+currency handles from T7MDefaults session scope — T7RTMVALID receives these pre-opened handles and uses them to verify that tax/multi-currency tables required by a selected RTM are accessible before launching the report; validator pattern: RTM selected→tax handle availability check→launch allowed/denied; NZLICE.LIB = licensed feature (only available when multi-currency/international licensed) — **C: 70/100**
- [x] 🔄 **SA** — Sales Analysis (13 ops) — 13 RWN programs fully identified: T7SAA(212p main engine: BKARINV+BKARINVL+BKARCUST+BKICMSTR+ISARCHG+ISSOBOX+BKPRSALE+ISJOB+CLASS+ISAREX); T7SAB/C/D/E/G/H/I/J/L (all 5p, range filter stubs, same table set as SAA); T7SAM(238p: BKSAREPT+BKACTRPT+ISBUILD+BKARINVL+BKARINV+BKICMSTR+ISRMAI+ISSRINFO+WORKORD+BKCMLEAD+BKCMTERR); T7SAN(220p: same as SAM excluding ISRMAI); T7SAO(169p Top N: BKCMACCL+BKCMACCC+ISAREX); T7SAP(131p class range: CLASMSTR+ISCATMST); T7SAQ(95p actual margin: WORKORD+WOMAT+WOBOM — uses actual WO costs); dedicated tables: BKSAREPT(57f full schema extracted: TYPE+NAME PK, RTM + 26 FROM/THRU range pairs covering inv#/dates/ship dates/amounts/salesperson/customer/class/category/part/lot/territory/currency), BKACTRPT(53f full schema extracted: TYPE+NAME PK, RTM + named FROM/THRU pairs for PART/CLASS/CAT/DATE/LOC/WO/CUST/INV/QC/LOT/SERIAL/PRICE/AVGC/STDC/DESC/REF/DEPT/QTY/SCRAP/VEND/PO/TYPE), ISJOB(9f job tracking: NUM/DESC/CUST/VEND/STATUS/OPENDT/CLOSEDT), ISAREX(51f AR extended: resale cert fields), ISRMAI(54f RMA invoice: NUM+PART+LINEID, STATUS/REASON/DISP/OSONUM/OINVNUM), BKCMACCL(2f account level), BKCMLEAD(2f lead source); Pass 157: 5 Java JARs confirmed in com.evoerp.salesanalysis.* namespace: SalesRepSummary.jar(srs)+ProfitByInvoice.jar(pbi)+ItemClass.jar(ic)+CustomerClass.jar(cc)+MultiYearSales.jar(multiyear) — SA analysis views are Java-backed — **C: 77/100**
...
- [x] 🔄 **QS** — Quick Sales Order / Web Order Staging (T7QSOA: 72 procs, 39t, src=EVO.LIB + T7QSOALines: 70 procs, 18t, src=LISTG60.LIB); ISQSOA(12f) schema extracted: IS_QSOA_UID(40) PK + CUST(10)+SHPTO(10)+SHPDTE+ITEM(15)+DESC(30)+QTY+PRICE+DISC+MDATE1/2+EXTRA(50); ISQSOA vars confirmed (Pass 115): IS.QSOA.CUST/DESC/EXTRA/ITEM/MDATE1/MDATE2/PRICE/QTY/SHPDTE/SHPTO/UID — all 12 fields accessed directly; T7QSOA opens LOT+SERIAL (lot/serial-controlled items in quick orders) + BKAPINVL+BKAPPO (AP cost check for pricing); T7QSOALines (70 procs) = SO line-detail viewer: opens BKCMACCT (CRM account context for line items) + BKICLOC+BKICPMAT (location+price matrix); ISTERMS+BKPRSALE confirm real SO creation; ISQSOA = staging record for quick/web order entry; Pass 160: **BKICREF** (item cross-reference table — new: maps customer/vendor part numbers to EVO item codes); **FOORDL.H** = ISFOORDL handle (FO order detail link — QS orders can originate from Features/Options configurator); **CAL.H** = calendar handle (delivery date calculation); **WC.H** = work center handle (lead time from routing); MCNTR3 = multi-counter for batch conversion; SSONUM = staging-to-SO conversion number — **C: 76/100**
- [x] 🔄 **VSCHED** — Visual Work Center Capacity Scheduler (T7VSCHED: 94 procs confirmed Pass 115, 22 tables); WCTRLOAD(8f: WC+DATE PK, TOTHRS+UDATE+CAP+UTIL+LOAD+EXTRA(100)); full DB fingerprint: WORKORD+WOROUT+WCTRLOAD+BKICMSTR+FILELOC+BKYSMSTR+BKARINV+BKARINVL+BKSYHELP+DBAHLPID+ISIS+MKAHIST+ISLOG+ISDRILL+BKAPVEND+BKARCUST+BKCMACCN+ISLINKS+BKAPDESC+ISACCESS+LANGDICT+BKSYMSTR; WCTRLOAD = pre-computed daily WC capacity snapshot (written by scheduler/MRP); BKARCUST+BKARINV+BKARINVL = customer demand overlay (open SO→WC); BKAPVEND+BKCMACCN = vendor/contact context for demand source; ISDRILL+ISLINKS+BKAPDESC = drill-through to source orders; ISACCESS = license gate; LANGDICT = multi-language UI; purpose confirmed = visual/Gantt WC capacity planning with customer demand overlay; Pass 157: WorkCenterLoad.jar (com.evoerp.wcload.javafx.App) confirmed as the Java-side visual scheduler — **C: 75/100**
...
| Module: QU/Query Tools | **82** | 75 | **0** ✅ ↑+7 Pass160 WBKLOOKUP FCFG.FILTERS/CBINDEXNAME/ICTVIEWABLE/BOGUS/LU_CONTROL_FLD/CURRENTFASTSRCH + ISDRILLM(17f) confirmed | 2026-06-22 |
...
| Subsystem: BS/Business Score | **84** | 82 | **0** ✅ ↑+2 Pass159 t7jbs.RWN (63p, LISTG60) = Java BS embedded panel confirmed | 2026-06-22 |
...
| Module: ML/Multi-Language | **82** | 76 | **0** ✅ ↑+6 Pass153 T7MLC+T7MLE fully read; LANG.DICT(ECAPT/LANG/LCAPT) confirmed; Generate/Edit/AddLang/Delete workflow confirmed; 3-char lang code format confirmed | 2026-06-22 |
...
| Module: FN/File Navigator | **80** | 72 | **0** ✅ ↑+8 Pass153 T7FNR.DFM fully read (3223 lines); IS.LOC(LOC_FILE_NAME/BUFF_NAME/LOCATION) + IS.DICT(DICT_FIELD_NAME/TYPE/SIZE/DESC) confirmed; 6 filter rows+POS[1-6]+spos+slength+TestFilters+PopupMenu confirmed | 2026-06-22 |
...
| Module: RT/RTM Validator | **70** | 55 | **0** ✅ ↑+15 Pass161 8 tax/currency handles: ISTXG/ISTXF/ISTAX/ISHTX/HTAX/ISMCF/ISMCR/ISDUTY; validates RTM template table accessibility at report launch | 2026-06-22 |
...
| Module: QS/Quick SO | **78** | 76 | **0** ✅ ↑+2 Pass160 BKICREF (cross-ref table); FOORDL.H (FO configurator link); CAL.H+WC.H delivery/lead-time calc | 2026-06-22 |
