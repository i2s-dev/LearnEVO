...
- [x] 🔄 **CH** — Multi-Location Chain (T7CHAIN: 62 procs, ISCHAINM+BKPSUSER+BKSYMSTR+LANGDICT; T7CHAINM: 40 procs, ISCHAINM+FILEDICT); ISCHAIN/ISCHAINM(17f identical)=USER(15)+PARENT(12)+CHILD(12) PK + PARAM_1..10(15 each)+AUTO(1)+DATE+DESC(100)+EXTRA(100); Pass 135 (2026-06-19): confirms both ISCHAIN+ISCHAINM exist (active dispatch + master/template pair); 2 programs confirmed; LANGDICT confirms multi-language chain deployments — **C: 75/100**
...
- [x] 🔄 All remaining tables not covered above — Pass 149 (2026-06-22): AHSYLOG(23f logon access array LEVL+MENU+CTRL+ACCES_1..20), ARTTEMP(12f AR payment temp staging CUST+TRXN PK), BKABCUST(5f license dates START/EXP/PERIOD/WARNING/STAND_ALNE), BKABVEND(2f license SERIAL+REG_NAME), BKACTRPT(53f activity report filter template TYPE+NAME PK, 25 FROM/THRU dimension pairs), BKFLDHLP(3f field help CODE+INDEX+LINE), BKFOCFG(18f features/options config MANFET+15×YN+OPCODE+EXTRA), BKISHTAX+BKISTAX(13f×2 identical IS tax history/current CODE+DATE PK), BKMATRIM(3f machine trim factors MACH+FIRST+SECND); plus DDF-level detail added for BKGLACHK/BKGLCHK/BKGLAGJL/BKGLAGJR/BKGLATRN/BKGLCCOA/BKGLCOA/BKLOGON/BKMATCST/BKMRPFC/BKMRPPO/BKMRPSW/BKESTQT/BKESTQTL — **C: 65/100** (significant portion of remaining tables still undocumented)
