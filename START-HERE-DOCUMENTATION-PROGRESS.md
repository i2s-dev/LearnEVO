...
| YN slot mapping (BKYSMSTR) | 🔄 Partial | 82/100 | **88 DFM-structural slots confirmed** (direct FieldName='BKYS.YN[N]' bindings in T7MDefNDC.DFM + T7MDEFAULTS.DFM — structural, not value-matched); 10 DBA.LIB functional descriptions (YN[205-215/224]); **Pass 575g: Frida 05c value-matches DISPROVEN** — company A vs F 05d scans: YN[88]=GLCTRL and YN[105]=WHCTRL are FALSE POSITIVES (ISTS values changed A→F but YN bytes did not); YN[201]=APLANG UNVERIFIED (same value both companies); effective confirmed drops 101→~88; BKYSMSTR confirmed system-wide (identical arrays both companies); all Frida/DFM/SRC/pointer/differential approaches exhausted; **only remaining path: T7YSYN.RWN bytecode disassembly via Ghidra**; see Issues/B-020 |
...
7. **✅ DONE (Pass 574k, 2026-07-07) Per-table field meaning documentation** — 579 tables / 21,299 fields
   from `Evo-DBA_File_Fields 052421.xlsx`; 33 per-module field files in `docs/04-data-dictionary/`;
   master table index in `docs/04-data-dictionary/table-index.md`. **100% field descriptions filled — 0 blank cells remain across all 33 fields-*.md files.**
