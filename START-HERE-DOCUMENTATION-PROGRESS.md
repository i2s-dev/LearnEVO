...
| YN slot mapping (BKYSMSTR) | 🔄 Partial | 88/100 | 101 slots confirmed total: 91 Frida+DFM+SRC (Pass 575c) + 10 DBA.LIB functional descriptions (Pass 575d); Pass 575e: pointer approach exhausted (ISTS vars packed in T7YSYN declaration order, not slot order); **Pass 575f: BKYSMSTR CONFIRMED SYSTEM-WIDE — Company B99 differential scan yielded 0 YN differences; 66 ISTS.CFG keys that differ between companies come from per-company table (not BKYSMSTR.YN[]); differential approach PERMANENTLY CLOSED**; ~149 slots unknown; **only remaining path: T7YSYN.RWN bytecode disassembly via Ghidra** |
